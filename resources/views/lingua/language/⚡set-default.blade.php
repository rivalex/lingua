...
new class extends Component {
	...
	public function mount(): void
	{
		...
		$this->confirm = Str::of(__('rivalex::lingua.languages.default.confirm',
...
<div>
	<flux:modal.trigger name="{{ $modalName }}">
		<flux:button icon="star" variant="primary" color="emerald"
		             size="xs">@lang('rivalex::lingua.languages.default.button')</flux:button>
	</flux:modal.trigger>
	<flux:modal name="{{ $modalName }}" class="translate-modal">
		<div class="flex flex-col gap-4" x-data="{ control: '' }">
			<h2 style="font-size: 1.5rem">@lang('rivalex::lingua.languages.default.header', ['language' => $language->name])</h2>
			<flux:separator/>
			<form wire:submit.prevent="setDefaultLanguage" id="setDefaultLanguage" class="flex flex-col gap-4">
				@csrf
				<p style="font-size: 0.9rem">@lang('rivalex::lingua.languages.default.alert', ['language' => $language->name])</p>
				<p style="font-size: 0.9rem">@lang('rivalex::lingua.global.confirm.change', ['confirm' => $confirm])</p>
				...
				<div class="flex justify-between gap-2 items-center">
					...
					<flux:button x-bind:type="control !== $wire.confirm ? 'button' : 'submit'"
						...
						@lang('rivalex::lingua.languages.default.action', ['language' => $language->name])
					</flux:button>
				</div>
			</form>
		</div>
