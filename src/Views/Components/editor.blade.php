...
<div @class([
    ...
    @if($type === 'text')
        <div data-flux-field>
            ...
            @if(!empty($label))
                <flux:label for="{{ $uid }}"
                    :badge="$required ? __('lingua::lingua.global.required') : null">{{ $label }}</flux:label>
            @endif
            <flux:textarea
                id="{{ $uid }}"
                {{ $attributes }}
        ...
        >
            @if(!empty($label))
                <flux:label x-bind:for="id"
                    :badge="$required ? __('lingua::lingua.global.required') : null">{{ $label }}</flux:label>
            @endif
            <ui-lingua-editor @if(config('lingua.editor.code-mode')) x-cloak x-show="!sourceView" @endif
                x-on:pointerdown.stop
                x-on:click.stop
            ...
            @if(config('lingua.editor.code-mode'))
                <div x-cloak x-show="sourceView">
                    <div class="toolbar" x-ref="toolbar">
                        <p class="source-code-label">{{ __('lingua::lingua.translations.editor.code-mode') }}</p>
    ...
    @endif
    @if($helper)
        <div class="lingua-editor-helper">
            @lang('lingua::lingua.translations.editor.helper_' . $type)
        </div>
    @endif
</div>
